...
Vagrant.configure("2") do |config|
  ...
  config.vm.define 'web' do |web|
   ...
   config.vm.provision "shell", inline: <<-SHELL
      sudo apt-get update
      sudo apt-get install -y python-pip python-dev libpq-dev postgresql postgresql-contrib git vim
      sudo pip install django flake8 psycopg2
      git clone https://github.com/mateusdemorais/GCS02.git
      python GCS02/manage.py migrate
      python GCS02/manage.py loaddata GCS02/db.fixture.json
   SHELL
end
